...
#Function to detect a face and autocrop
def facecrop(image):
    ...
    for f in faces:
        ...
        sub_face = img[y-math.floor(0.50*h):y+h+math.floor(0.40*h), x-math.floor(0.20*w):x+w+math.floor(0.20*w)]#cropping the image
...
@requires_auth
def index():
   return render_template('ImagePreview.html')#Load HTML page

#Function to return the number of images in a folder
@app.route('/count')
def countImages():
...
def compute():
    if request.method == 'POST':
        ...
        im=im.convert('RGB')#Convert image to RBG color space
        pix=im.load()
        width,height=im.size#image dimensions
        for x in range(width):#traverse each pixel horizontally
            for y in range(height):#traverse each pixel vertically
                r, g, b = pix[x, y]
                h_ratio, s_ratio, v_ratio = rgb_to_hsv(r / 255.0, g / 255.0, b / 255.0)#function call to convert RGB values
                h, s, v = (h_ratio * 360, s_ratio * 255, v_ratio * 255)
                min_h, min_s, min_v = GREEN_RANGE_MIN_HSV
                max_h, max_s, max_v = GREEN_RANGE_MAX_HSV
                if min_h <= h <= max_h and min_s <= s <= max_s and min_v <= v <= max_v:#check if pixel color is within the range
                    pix[x, y] = (255, 255, 255,1)#set pixel value to white
        im.save(os.path.join(app.config['PROCESSED_IMAGES'], filename))#save processed image
...
if __name__=='__main__':
	app.run(debug=True)#remove when deploying
